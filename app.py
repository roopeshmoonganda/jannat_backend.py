...
FYERS_REDIRECT_URI = os.environ.get("FYERS_REDIRECT_URI", "http://localhost:5000/fyers_auth_callback") # Must match your Fyers app settings
FYERS_CLIENT_ID = os.environ.get("FYERS_CLIENT_ID", f"{FYERS_APP_ID}-100" if FYERS_APP_ID != "YOUR_APP_ID_FROM_FYERS" else "") # Example: "YOUR_APP_ID-100"
...
def check_fyers_client_initialized():
    """Ensures fyersClient is initialized before processing requests."""
    global fyers
    if fyers is None:
...
def generate_auth_url():
    ...
    try:
        session = fyersClient.SessionModel(
...
def validate_credentials():
    ...
    try:
        test_fyers = fyersClient.FyersClient(token=access_key, is_async=False, client_id=client_id, log_path=os.getcwd())
        ...
        if profile_data and profile_data.get('s') == 'ok':
            ...
            global fyers
            fyers = test_fyers # Set the global Fyers client
    ...
    # --- Live Trading Logic ---
        ...
        try:
            ...
            order_data = {
                ...
                "qty": int(quantity),
                "type": 2,  # 2 for MARKET order, 1 for LIMIT order
                "side": side,
                "productType": product_type, # e.g., "MIS"
                "limitPrice": 0, # For MARKET order
                "stopPrice": 0, # For MARKET order
                "validity": "DAY",
                "disclosedQty": 0,
                "offlineOrder": "False",
                "Hedge": {},
            ...
            if order_type == "LIMIT":
                order_data["type"] = 1
                order_data["limitPrice"] = float(entry_price) # Use entryPrice as limit price
                order_data["stopPrice"] = 0 # No stop price for simple limit order
